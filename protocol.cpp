  ...
  if(m_w2m_controlMcu.sub_cmd == SUB_CMD_CONTROL_MCU)
  {
    ...
    if((m_w2m_controlMcu.cmd_tag[2] & 0x01) == 0x01)
    {
		...
		if((m_w2m_controlMcu.status_w.cmd_byte[1] & 0x01) == 0x01)
		{
			...
			#if MYSERIAL_DATA
			mySerial.write("FF130000FF");//向小车发送手动模式的对应指令
			#endif
		}
		else
		{
